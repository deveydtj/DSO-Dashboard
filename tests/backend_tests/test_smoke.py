...
class TestSloConfigSmoke(unittest.TestCase):
    ...
    def test_load_config_slo_numeric_string_from_config_json(self):
        """Test that numeric string SLO values from config.json are parsed correctly"""
        # This can happen when config.json is generated by a templating system
        config_data = {
            'gitlab_url': 'https://gitlab.com',
            'api_token': 'test',
            'slo': {
                'default_branch_success_target': '0.95'  # String, not float
            }
        }
        ...
        with patch('os.path.exists', return_value=True):
            with patch('builtins.open', unittest.mock.mock_open(read_data=json.dumps(config_data))):
                ...
                # Should be parsed to float
                self.assertEqual(config['slo']['default_branch_success_target'], 0.95)
                self.assertIsInstance(config['slo']['default_branch_success_target'], float)
